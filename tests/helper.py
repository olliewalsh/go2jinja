...
def run_test(test_name: str, input_file: str, go_file: str = ""):
    ...
    gotmpl_file = os.path.join(get_test_directory(), test_name, f"{test_name}.gotmpl" if go_file == "" else go_file)
    ...
    go_out = Command(f"{os.path.join(get_test_directory(), "gotemplate", "gotemplate")} -data-file={data_file} -go-template={gotmpl_file}").run()
    ...
    for _, test_data in data.items():
        try:
            rtemplate = Environment(loader=BaseLoader, keep_trailing_newline=True).from_string(jinjatmpl)
