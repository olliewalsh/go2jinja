...
class FunctionNode:
    ...
    def to_jinja(self) -> str:
        if self.type in [FunctionType.EQUALS,
                         FunctionType.NEQUALS, 
                         FunctionType.GREATER, 
                         FunctionType.GREATEREQUALS, 
                         FunctionType.LESSER, 
...
def parse_go_template(content: str) -> list[Node]:
    ...
    while start_pos != -1:
        ...
        if start_pos - end_pos > 0:
            content_node = Node(end_pos, start_pos, content[end_pos:start_pos], NodeType.CONTENT, None, None, current_scope_node)
        ...
        expr_node = Node(start_pos, end_pos, content[start_pos:end_pos], node_type, prev_expr_node, None, None)
    ...
    if end_pos < len(content):
        content_node = Node(end_pos, len(content)+1, content[end_pos:len(content)+1], NodeType.CONTENT, None, None, current_scope_node)
        current_scope_node = None if not current_scope_nodes else current_scope_nodes[-1]
        if current_scope_node is not None:
            current_scope_node.children.append(content_node)
        else:
...
def is_go_template(content: str) -> bool:
    return re.compile(R"{{\-?.+\-?}}").match(content) is not None and \
        re.compile(R"{%\-?.+\-?%}").match(content) is None
...
def go_to_jinja(content: str) -> str:
    ...
    def node_to_jinja_str(node: Node) -> str:
        if node.type == NodeType.STATEMENT:
            ...
            if m is not None:
                content = transform_go_var_to_jinja(node.content[m.start(1):m.ends(1)[0]])
                content = transform_go_local_var_to_jinja(content)
                content = node.content[:m.start(1)] + content + node.content[m.ends(1)[0]:]
                return content. \
                    replace(GO_SYMBOL_OPEN_BRACKETS, JINJA_SYMBOL_STMT_OPEN_BRACKETS). \
                    replace(GO_SYMBOL_CLOSE_BRACKETS, JINJA_SYMBOL_STMT_CLOSE_BRACKETS)
        elif node.type == NodeType.IF:
            m = GO_KEYWORDS[NodeType.IF].match(node.content)
            if m is not None and len(m.groups()) == 2:
                pipeline = m.groups()[1].strip()
                return node.content.replace(pipeline, parse_pipeline(pipeline)). \
            ...
            while prev is not None:
                ...
                if closing_count == 0:
                    ...
                    if prev.type in [NodeType.IF, NodeType.ELIF, NodeType.ELSE]:
                        return node.content[:m.start(1)].replace(GO_SYMBOL_OPEN_BRACKETS, JINJA_SYMBOL_OPEN_BRACKETS) + \
                                "endif" + \
                                node.content[m.ends(1)[0]:].replace(GO_SYMBOL_CLOSE_BRACKETS, JINJA_SYMBOL_CLOSE_BRACKETS)
            ...
            if m is not None and len(m.groups()) == 2:
                ...
                pipeline = m.groups()[1].strip()
                return node.content.\
                        replace(variable, f"set {transform_go_local_var_to_jinja(variable)}", 1). \
            ...
            if m is not None:
                ...
                content = node.content. \
                    replace("range", f"for {loop_var[0]} in"). \
...
def tree_structure(nodes: list[Node], level: int) -> str:
    res = ""
    for node in nodes:
        res += level* "\t" + f"{node.type}: {node.start},{node.end} - {len(node.children)} - {node.content}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--go-template", dest="template", required=True, help="Path to the file containing the Go Template to convert to Jinja")
    parser.add_argument("--output", dest="output", default="", help="Output file path for the converted Jinja Template. If empty, prints to stdout.")
